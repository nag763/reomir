...
from main import (_create_autoclose_html_response, _encrypt_data_kms,
                  _get_auth_user_info, app)
...
def test_github_connect_success(
    client, monkeypatch, mock_firestore_client_constructor, mock_kms_client_constructor
):
    user_info_payload = {"sub": "test_user"}
    encoded_user_info = base64.b64encode(
        json.dumps(user_info_payload).encode("utf-8")
    ).decode("utf-8")
...
def test_github_connect_missing_header(
    client, monkeypatch, mock_firestore_client_constructor, mock_kms_client_constructor
):
    monkeypatch.setattr(main, "db", mock_firestore_client_constructor.return_value)
    monkeypatch.setattr(main, "KMS_CLIENT", mock_kms_client_constructor.return_value)
...
def test_github_connect_missing_client_id(
    client, monkeypatch, mock_firestore_client_constructor, mock_kms_client_constructor
):
    user_info_payload = {"sub": "test_user"}
    encoded_user_info = base64.b64encode(
    ...
    with mock.patch.object(
        main, "API_GATEWAY_BASE_URL", "https://api.example.com"
...
def test_github_connect_missing_base_url(
    client, monkeypatch, mock_firestore_client_constructor, mock_kms_client_constructor
):
    user_info_payload = {"sub": "test_user"}
    encoded_user_info = base64.b64encode(
...
def test_encrypt_data_kms_success(monkeypatch, mock_kms_client_constructor):
    ...
    monkeypatch.setattr(
        main, "KMS_CLIENT", mock_kms_instance
    )  # Ensure main.KMS_CLIENT is this mock

    with mock.patch.object(
        main, "GOOGLE_CLOUD_PROJECT", "test_project"
    ), mock.patch.object(main, "KMS_LOCATION", "global"), mock.patch.object(
        main, "KMS_KEY_RING", "test_key_ring"
    ), mock.patch.object(
        main, "KMS_KEY_NAME", "test_key"
    ):
...
def test_github_callback_success(
    ...
    with mock.patch.multiple(main, **COMMON_CALLBACK_ENV_PATCHES):
        mock_requests.post(
            ...
            json={"access_token": "test_github_token"},
        )
        ...
        mock_requests.get(
            "https://api.github.com/user", json={"login": "test_github_user", "id": 123}
        )

        response = client.get("/api/v1/github/callback?code=test_code&state=test_user")

        assert response.status_code == 200
        assert "Success! You have been authenticated." in response.get_data(
            as_text=True
        )
...
def test_github_callback_missing_oauth_env_vars(
    client, monkeypatch, mock_firestore_client_constructor, mock_kms_client_constructor
):
    monkeypatch.setattr(main, "db", mock_firestore_client_constructor.return_value)
    monkeypatch.setattr(main, "KMS_CLIENT", mock_kms_client_constructor.return_value)
    ...
    with mock.patch.multiple(main, **temp_env), mock.patch.object(
        main, "GITHUB_CLIENT_ID", None
    ):  # Ensure it's None for the test
        response = client.get("/api/v1/github/callback?code=test_code&state=test_user")
    assert response.status_code == 200
    assert "Server configuration error" in response.get_data(as_text=True)


def test_github_callback_token_exchange_error(
    client,
    mock_requests,
    monkeypatch,
    mock_firestore_client_constructor,
    ...
    with mock.patch.multiple(main, **COMMON_CALLBACK_ENV_PATCHES):
        mock_requests.post(
            "https://github.com/login/oauth/access_token", status_code=500
        )
        response = client.get("/api/v1/github/callback?code=test_code&state=test_user")
    assert response.status_code == 200
...
def test_github_callback_no_access_token_in_response(
    client,
    mock_requests,
    monkeypatch,
    mock_firestore_client_constructor,
...
def test_github_callback_kms_encryption_error(
    client,
    mock_requests,
    monkeypatch,
    mock_firestore_client_constructor,
    ...
    with mock.patch.multiple(main, **COMMON_CALLBACK_ENV_PATCHES):
        ...
        response = client.get("/api/v1/github/callback?code=test_code&state=test_user")
    assert response.status_code == 200
    assert "A security error occurred during processing." in response.get_data(
        as_text=True
    )


def test_github_callback_get_user_info_error(
    client,
    mock_requests,
    monkeypatch,
    mock_firestore_client_constructor,
...
def test_github_callback_missing_user_info_fields(
    client,
    mock_requests,
    monkeypatch,
    mock_firestore_client_constructor,
    ...
    with mock.patch.multiple(main, **COMMON_CALLBACK_ENV_PATCHES):
        ...
        response = client.get("/api/v1/github/callback?code=test_code&state=test_user")
    assert response.status_code == 200
    assert "Could not retrieve user profile from GitHub." in response.get_data(
        as_text=True
    )


def test_github_callback_firestore_error(
    client,
    mock_requests,
    monkeypatch,
    mock_firestore_client_constructor,
    ...
    with mock.patch.multiple(main, **COMMON_CALLBACK_ENV_PATCHES):
        ...
        mock_db_instance.collection("users").document("test_user").set.side_effect = (
...
def test_github_status_connected(
    client, monkeypatch, mock_kms_client_constructor, mock_firestore_doc_setup
):
    monkeypatch.setattr(
        main, "KMS_CLIENT", mock_kms_client_constructor.return_value
    )  # Not used by status, but good practice
...
def test_github_status_not_connected(
    client, monkeypatch, mock_kms_client_constructor, mock_firestore_doc_setup
):
    monkeypatch.setattr(main, "KMS_CLIENT", mock_kms_client_constructor.return_value)
    mock_doc_ref, mock_doc_snapshot = mock_firestore_doc_setup
...
def test_github_status_doc_not_exists(
    client, monkeypatch, mock_kms_client_constructor, mock_firestore_doc_setup
):
    monkeypatch.setattr(main, "KMS_CLIENT", mock_kms_client_constructor.return_value)
    mock_doc_ref, mock_doc_snapshot = mock_firestore_doc_setup
...
def test_github_status_missing_header(
    client, monkeypatch, mock_kms_client_constructor, mock_firestore_client_constructor
):
    monkeypatch.setattr(main, "db", mock_firestore_client_constructor.return_value)
    monkeypatch.setattr(main, "KMS_CLIENT", mock_kms_client_constructor.return_value)
...
def test_github_status_firestore_error(
    client, monkeypatch, mock_kms_client_constructor, mock_firestore_client_constructor
):
    mock_db_instance = mock_firestore_client_constructor.return_value
    monkeypatch.setattr(main, "db", mock_db_instance)
    ...
    mock_db_instance.collection("users").document("test_user").get.side_effect = (
        Exception("Firestore error")
    )
...
def test_github_disconnect_success(
    client, monkeypatch, mock_kms_client_constructor, mock_firestore_doc_setup
):
    monkeypatch.setattr(main, "KMS_CLIENT", mock_kms_client_constructor.return_value)
    mock_doc_ref, mock_doc_snapshot = mock_firestore_doc_setup
    ...
    mock_doc_snapshot.exists = True
    mock_doc_snapshot.get = (
        lambda key: {"github_connected": True}.get(key)
...
def test_github_disconnect_not_connected(
    client, monkeypatch, mock_kms_client_constructor, mock_firestore_doc_setup
):
    monkeypatch.setattr(main, "KMS_CLIENT", mock_kms_client_constructor.return_value)
    mock_doc_ref, mock_doc_snapshot = mock_firestore_doc_setup
...
def test_github_disconnect_missing_header(
    client, monkeypatch, mock_kms_client_constructor, mock_firestore_client_constructor
):
    monkeypatch.setattr(main, "db", mock_firestore_client_constructor.return_value)
    monkeypatch.setattr(main, "KMS_CLIENT", mock_kms_client_constructor.return_value)
...
def test_github_disconnect_firestore_error(
    client, monkeypatch, mock_kms_client_constructor, mock_firestore_doc_setup
):
    monkeypatch.setattr(main, "KMS_CLIENT", mock_kms_client_constructor.return_value)
    mock_doc_ref, mock_doc_snapshot = mock_firestore_doc_setup
