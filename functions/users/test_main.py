...
def mock_firestore_client_instance_control(request):
    ...
    if not isinstance(mock_db_instance, mock.Mock):
        pytest.fail(f"Patching error: main_module.db is type {type(mock_db_instance)}, not unittest.mock.Mock. "
        ...
        def get_json_side_effect(silent=False):
            if data is None:
                if not silent:
                    ...
                    raise BadRequest("Failed to decode JSON object: Expecting value: line 1 column 1 (char 0)")
                return None
            return data
...
def test_post_user_valid_data(mock_request, mock_firestore_client_instance_control):
    ...
    expected_stored_data = {
        "uid": user_id, "email": user_email, "displayName": user_name,
...
def test_post_user_invalid_cookie_consent(mock_request, mock_firestore_client_instance_control):
    mock_db = mock_firestore_client_instance_control
    ...
    request_obj_missing = mock_request("POST", data={"otherField": True}, headers=headers)
    response_missing, status_code_missing, _ = handler(request_obj_missing)
    assert status_code_missing == 400
    assert response_missing == {"error": "'cookieConsent' field must be present and set to 'true'."}

    request_obj_wrong_type = mock_request("POST", data={"cookieConsent": True}, headers=headers)
    response_wrong_type, status_code_wrong_type, _ = handler(request_obj_wrong_type)
    assert status_code_wrong_type == 400
    assert response_wrong_type == {"error": "'cookieConsent' field must be present and set to 'true'."}

    request_obj_false_str = mock_request("POST", data={"cookieConsent": "false"}, headers=headers)
    response_false_str, status_code_false_str, _ = handler(request_obj_false_str)
    assert status_code_false_str == 400
    assert response_false_str == {"error": "'cookieConsent' field must be present and set to 'true'."}
...
def test_post_user_no_json_body(mock_request, mock_firestore_client_instance_control):
    ...
    user_info_json = json.dumps(user_info)
    user_info_b64 = base64.b64encode(user_info_json.encode('utf-8')).decode('utf-8')
    headers = {"X-Apigateway-Api-Userinfo": user_info_b64, "Content-Type": "application/json"}
    ...
    assert "Failed to decode JSON object" in response["error"] or "Invalid JSON payload" in response["error"]
    mock_db.collection.assert_not_called()
...
def test_malformed_auth_header_not_base64(mock_request, mock_firestore_client_instance_control):
    mock_db = mock_firestore_client_instance_control
...
def test_malformed_auth_header_not_json(mock_request, mock_firestore_client_instance_control):
    mock_db = mock_firestore_client_instance_control
...
def test_auth_header_missing_user_id_claim(mock_request, mock_firestore_client_instance_control):
    mock_db = mock_firestore_client_instance_control
    ...
    assert status_code == 400
    assert response == {"error": "User ID claim ('sub') not found in authentication information."}
    mock_db.collection.assert_not_called()
